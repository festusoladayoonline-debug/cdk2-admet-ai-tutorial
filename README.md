...
├── README.md                          ← This file
├── cdk2\\\_admet\\\_tutorial4\\\_v3.ipynb      ← Main tutorial notebook
├── data/
│   └── cdk2\\\_inhibitors\\\_200.csv        ← 200 CDK2 inhibitors (ChEMBL 35, curated)
└── outputs/                           ← Generated by notebook (not tracked by git)
    ├── cdk2\\\_admet\\\_raw\\\_predictions.csv
    ├── cdk2\\\_integrated\\\_admet\\\_report.csv
    └── \\\*.png / \\\*.svg
```
The dataset (`data/cdk2\\\_inhibitors\\\_200.csv`) is provided as a standalone CSV file — data and code are kept separate following best practice. The notebook reads from this file; no data is hardcoded in the notebook itself.
---
...
```
This creates the `cdk2\\\_admet\\\_tutorial` conda environment with all required packages, including ADMET-AI 2.0.1 installed via pip (ADMET-AI is not available on conda-forge; all other packages use conda-forge to minimise dependency conflicts).
...
chembl-webresource-client	0.10.9	pip (PyPI)
> \\\*\\\*Why numpy < 2.0?\\\*\\\* RDKit conda-forge builds for Python 3.11 are compiled against NumPy 1.x. Using numpy ≥ 2.0 causes an `AttributeError: \\\_ARRAY\\\_API not found` crash at RDKit import. The constraint `numpy>=1.26.4,<2.0` resolves this automatically.
...
```bash
conda activate cdk2\\\_admet\\\_tutorial
python -c "import admet\\\_ai; from rdkit import rdBase; print('admet\\\_ai:', admet\\\_ai.\\\_\\\_version\\\_\\\_); print('rdkit:', rdBase.rdkitVersion)"
```
Expected output:
```
admet\\\_ai: 2.0.1
...
```bash
conda activate cdk2\\\_admet\\\_tutorial
python -m ipykernel install --user --name cdk2\\\_admet\\\_tutorial --display-name "Python (cdk2\\\_admet\\\_tutorial)"
cd /path/to/cdk2-admet-tutorial
jupyter notebook
```
Open `cdk2\\\_admet\\\_tutorial4\\\_v3.ipynb` in the browser tab. Confirm the kernel in the top-right corner shows `Python (cdk2\\\_admet\\\_tutorial)`. Run cells sequentially from Cell 0 to Cell 15 — do not skip cells as each cell defines variables used by subsequent cells.
---
Tested Platforms
...
Data separated from code — dataset in `data/cdk2\\\_inhibitors\\\_200.csv`
...
@article{Anebi2025cdk2admet,
  ...
  author  = {Festus O. Ogungbemiro1, Jane O. Anebi, Felix O. Okunlola, Love A. Onyekwere and Evna      
